...
class AreaReportList extends Component  {
  ...
  AreaAggregator = function(sections,sectionProps,sectionTypes){
    ...
    let areaAdder = (sectionProp) => {
        ...
        return area(sections[sectionProp.id])*sectionProp.floors
    }
    ...
    let buildingAreas = buildingList.map(buildingMaker)
    console.log(buildingList)
    console.log(buildingAreas)
    return R.zipWith((x,y)=>{return {building:x, area:y}},buildingList,buildingAreas)
    }

  // console.log(denormalizedData.users)
  render (){ 
    console.log(this.AreaAggregator())
    return(
      <Row>
        <div>
          <BootstrapTable 
          ...
          hover
          striped
          data = {this.AreaAggregator(this.props.sections,this.props.sectionProps,this.props.sectionTypes)}
        //   search
          condensed>
              <TableHeaderColumn isKey dataField='building'>Buildings</TableHeaderColumn>
...
export default connect(mapStateToProps)(AreaReportList);